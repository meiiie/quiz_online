...
export default function MainLayout({ children }: MainLayoutProps) {
  ...
  const topPadding = !isHomePage && !isStudentRoute ? (isMobile ? "pt-20" : "pt-28") : ""

  return (
    <div className="flex flex-col min-h-screen">
      {!isStudentRoute && <Header />}

      {/* Main content with mobile-optimized padding */}
      <main className={`flex-grow ${topPadding} ${isStudentRoute ? "h-screen" : ""}`}>{children || <Outlet />}</main>
